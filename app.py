...
app = Flask(__name__)
LOT = 65
state = {"ist":"", "ltp":0, "ema3":0, "ema5":0, "c1_3":"Waiting 9:15 Tomorrow", "c2_3":"Waiting 9:18", "call3":"Ready Tomorrow", "put3":"Ready Tomorrow", "call5":"Ready Tomorrow", "put5":"Ready Tomorrow", "status":"READY + LTP FIX", "msg":"Starting..."}
...
except Exception as e: state["msg"]=f"Login Fail {e}"; print(e)

def get_ltp():
    try:
        # Try all methods
        for ex, sym, token in [("NSE","Nifty 50","26000"), ("NSE","NIFTY","99926000"), ("NSE","Nifty 50","26000")]:
            try:
        ...
        # Last try - getQuote
        try:
            q=smart.getCandleData({"exchange":"NSE","symboltoken":"26000","interval":"ONE_MINUTE","fromdate":"2026-09-08 12:00","todate":"2026-09-08 12:30"})
            if q and 'data' in q and q['data']:
                return float(q['data'][-1][4])
        except Exception as e:
            state["msg"]=f"Quote Err {e}"
    except Exception as e:
...
def worker():
    ...
    while True:
        try:
            ...
            state["ltp"]=ltp
            if ltp==0:
                state["status"]=f"⚠️ LTP 0 - Market Closed? or Token Wrong? Time {ist.strftime('%H:%M')} - Check Angel One App Nifty LTP"
            ...
            if second3: state["c2_3"]=f"2nd 3Min H:{second3[1]} C:{second3[3]} Break>{first3[1] if first3 else 0}"

            if mod==554: closes_3.clear(); closes_5.clear(); first3=None; second3=None; first5=None; second5=None
            ...
            if first3 and second3 and len(closes_3)>=2:
                ...
                if c1>o1 and c2>o2 and (h2>h1 or c2>h1):
                    ...
                    state["call3"]=f"🟢 CALL 3MIN + 20EMA BUY CE @ {entry} SL {sl} TGT {tgt} 1:2 LOT {LOT} EMA {ema3} LTP {ltp}"
                    state["status"]=f"🟢 CALL SETUP READY + EMA {ema3}"
                if c1<o1 and c2<o2 and (l2<l1 or c2<l1):
                    sl=h1; tgt=entry-(sl-entry)*2
                    state["put3"]=f"🔴 PUT 3MIN + 20EMA BUY PE @ {entry} SL {sl} TGT {tgt} 1:2 LOT {LOT} EMA {ema3} LTP {ltp}"
                    state["status"]=f"🔴 PUT SETUP READY + EMA {ema3}"
            ...
            state["msg"]=f"Worker Err {e}"; time.sleep(2)
