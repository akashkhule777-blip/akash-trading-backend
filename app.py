...
def get_spot():
 try:
  if smart:
   ...
   v=float(d['data']['ltp'])
   if v>1000: return v
   if state["ltp"]>1000: return state["ltp"]
 except: pass
 try:
  ...
  if r.status_code==200:
   for idx in r.json()['data']:
    if idx['index']=='NIFTY 50': return float(idx['last'])
 except: pass
  ...
  try:
   ...
   if phase3==2 and first3:
    ...
    if abs(spot-fifty)<=15:
     ce,pe,strike,exp=get_atm_details(spot)
...
@app.route('/')
def home():
 return f"<h1 style='background:green;color:white;padding:8px'>AUTO ORDER ON</h1><h2>NIFTY {state['ltp']} | {state['atm']} | {state['ist']}</h2><h3>3MIN {state['c3']} Cnt {state['cnt3']}</h3><h2 style='color:green'>{state['call3']}</h2><h2 style='color:red'>{state['put3']}</h2><hr><h3>5MIN {state['c5']} Cnt {state['cnt5']}</h3><h2 style='color:green'>{state['call5']}</h2><h2 style='color:red'>{state['put5']}</h2><h4>{state['msg']}</h4>"
@app.route('/check')
