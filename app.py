...
app = Flask(__name__)
LOT=65
state={"ist":"","ltp":0,"c3":"Wait OB","call3":"Scanning...","put3":"Scanning...","atm":"Wait","cnt3":0,"msg":"PROXY FIX ON"}
smart=None
...
try:
 ...
 state["msg"]="Login OK - Proxy LTP ON"
...
def get_spot():
 ...
 # 1. ALLORIGINS PROXY - Yahoo - 100% works on Render
 try:
  r=requests.get("https://api.allorigins.win/raw?url=https://query1.finance.yahoo.com/v8/finance/chart/%5ENSEI",timeout=8)
  if r.status_code==200:
   js=r.json()
   price=js['chart']['result'][0]['meta']['regularMarketPrice']
   if price>1000:
    state["msg"]=f"Yahoo Proxy OK {price}"
    return float(price)
 except Exception as e:
  state["msg"]=f"Proxy Err {e}"

 # 2. Moneycontrol direct
 try:
  r=requests.get("https://priceapi.moneycontrol.com/pricefeed/nse/indices/NIFTY%2050",headers={"User-Agent":"Mozilla/5.0"},timeout=5)
  if r.status_code==200:
   js=r.json()
   price=float(js['data']['pricecurrent'])
   if price>1000:
    return price
 except: pass

 # 3. Angel LTP fallback
 try:
  if smart:
   d=smart.ltpData("NSE","Nifty 50","99926000")
   if d and d['data'] and d['data'].get('ltp'):
    lv=float(d['data']['ltp'])
    if lv>1000:
     return lv
 except: pass
...
@app.route('/')
def home():
 return f"<h1 style='background:green;color:white;padding:8px'>AUTO ORDER ON - PROXY FIX</h1><h2>NIFTY {state['ltp']} | {state['atm']} | {state['ist']}</h2><h3>3MIN {state['c3']} Cnt {state['cnt3']}</h3><h2 style='color:green'>{state['call3']}</h2><h2 style='color:red'>{state['put3']}</h2><h4>{state['msg']}</h4>"
@app.route('/check')
