...
except Exception as e: state["msg"]=f"Login Fail {e}"; state["status"]=f"Login Fail {e}"

def get_ltp():
    try:
        if smart:
            d=smart.ltpData("NSE", "Nifty 50", "26000")
            if d and isinstance(d, dict) and 'data' in d and d['data']:
                price = d['data'].get('ltp')
                if price:
                    l=float(price)
                    if l>1000:
                        state["msg"]=f"Angel LTP {l}"
                        return l
    except: pass
    try:
        ...
        if p and float(p)>1000:
            state["msg"]=f"Yahoo LTP {p}"
            return float(p)
    except Exception as e: state["msg"]=f"Yahoo fail {e}"
    return 0
...
def worker():
    ...
    while True:
        try:
            ...
            ltp=get_ltp(); state["ltp"]=ltp
            if ltp==0: time.sleep(3); continue
            mod=ist.hour*60+ist.minute; s3=(mod//3)*3
            ...
            ema3=calc_ema(closes_3,20); ema5=calc_ema(closes_5,20); state["ema3"]=ema3; state["ema5"]=ema5
            ...
            time.sleep(2)
        except Exception as e: state["msg"]=f"Worker {e}"; time.sleep(2)

threading.Thread(target=worker, daemon=True).start()

@app.route('/')
def home():
    return f"<h1 style='background:green;color:white;padding:12px'>FINAL REAL + 20EMA + CALL PUT BOTH + 3MIN 5MIN + LOT 65</h1><h2>{state['status']}</h2><h2 style='color:green'>CALL 3MIN: {state['call3']}</h2><h2 style='color:red'>PUT 3MIN: {state['put3']}</h2><h2 style='color:green'>CALL 5MIN: {state['call5']}</h2><h2 style='color:red'>PUT 5MIN: {state['put5']}</h2><h3>{state['c1_3']}</h3><h3>{state['c2_3']}</h3><h3>LTP {state['ltp']} EMA3:{state['ema3']} EMA5:{state['ema5']} {state['ist']}</h3><h4>{state['msg']}</h4>"
@app.route('/check')
def check(): return jsonify(state)
if __name__=="__main__": app.run(host='0.0.0.0',port=10000)from flask import Flask, jsonify
import os, threading, time, pyotp, requests
from datetime import datetime, timedelta
from SmartApi import SmartConnect
from collections import deque
