...
def find_ob(c):
    ...
    box=c[-16:-2]
    return {"high":max(float(x[2]) for x in box),"low":min(float(x[3]) for x in box),"50":(max(float(x[2]) for x in box)+min(float(x[3]) for x in box))/2}
def get_candles(smart, token):
    for interval in ["ONE_MINUTE","THREE_MINUTE","FIVE_MINUTE","ONE_DAY"]:
        try:
            frm=(ist_now()-timedelta(days=7)).strftime("%Y-%m-%d %H:%M")
...
def parse_search(res):
    try:
        if isinstance(res,str): import json as js; res=js.loads(res)
        if res.get('data') and len(res['data'])>0: return res['data'][0]
    except: pass
    return None
def run():
    try:
    ...
    while True:
        try:
            ...
            ce_data,ce_int=get_candles(smart,ce_tok);time.sleep(0.5)
            ...
            msgs=[]
            if ce_ob and not ce_done and float(ce_data[-1][4])>ce_ob["high"]:
                ...
                msgs.append(f"REAL CE BUY {d['ce_50']}");ce_done=True
            if pe_ob and not pe_done and float(pe_data[-1][4])>pe_ob["high"]:
                ...
                msgs.append(f"REAL PE BUY {d['pe_50']}");pe_done=True
            d["action"]=" | ".join(msgs) if msgs else f"Wait | {d['log']}"
...
def home():
    d=load()
    return f"<html><head><meta http-equiv='refresh' content='10'><meta name='viewport' content='width=device-width'><style>body{{background:#111;color:#fff;font-family:Arial;padding:10px}}.g{{color:#0f0}}.y{{color:#ffeb3b}}.b{{border:1px solid #444;padding:8px;border-radius:8px;margin:6px 0;background:#1a1a1a}}</style></head><body><h2 class=g>NIFTY {d['nifty']} ATM {d['atm']}</h2><div class=b>CE {d['ce_sym']} BOX {d['ce_l']}-{d['ce_h']} 50% {d['ce_50']} SL {d['ce_sl']}</div><div class=b>PE {d['pe_sym']} BOX {d['pe_l']}-{d['pe_h']} 50% {d['pe_50']} SL {d['pe_sl']}</div><div class=b>{d['exp']} | {d['log']}</div><h3 class=y>{d['action']}</h3><p>{d['time_str']} | 65 REAL</p></body></html>"
